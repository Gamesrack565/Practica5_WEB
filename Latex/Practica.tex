...
\begin{titlepage}
    ...
    \begin{spacing}{1.5}
        ...
        {\LARGE FRONTEND PARA GESTIÓN DE UNA LIBRERÍA}
...
Se busca crear una API web cuyo proposito sea la digitalizacion y gestion de forma persistente del inventario
de una libreria estandar.
Para la gestión del inventario, se debe garantizar lo siguiente:
\begin{itemize}
    \item Permitir crear, leer, actualizar y borrar libros. autores, editoriales y categorías mediante una base de datos.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{endpoints (URLs):} Para crear, leer, actualizar y borrar libros, autores, editoriales, etc.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Imagenes/Esquemas 1.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.83\textwidth]{Imagenes/Esquemas 2.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Imagenes/Esquemas 3.png}
\end{figure}
\newpage
...
\subsubsection*{\texttt{Rutas/}: Controladores de Endpoints de la API}
...
\subsubsection*{Carga y Renderizado de Datos}
La función \texttt{cargarLibros} es el núcleo de la interfaz. Utiliza la API \texttt{Fetch} de JavaScript para solicitar los datos al servidor
 de forma asíncrona (\texttt{async/await}). Esto permite que la interfaz no se congele mientras espera la respuesta de la base de datos. 
 Una vez recibidos los datos en formato JSON, se iteran y se inyectan dinámicamente en el DOM HTML.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/front c_1.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/front c_2.png}
\end{figure}

\subsubsection*{Construcción Dinámica de URLs}
Para conectar con los múltiples endpoints de búsqueda que ofrece la API (por título, autor, categoría, etc.), se implementó una lógica de 
construcción de URLs basada en el filtro seleccionado por el usuario. Esto asegura que la petición HTTP coincida exactamente con lo que espera
 el backend
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/front c_3.png}
\end{figure}

\subsubsection*{Optimización de Búsqueda (Debounce)}
Para evitar saturar el servidor con una petición por cada tecla que presiona el usuario, se implementó un mecanismo de 
\textit{debounce}. Esta función espera a que el usuario deje de escribir durante 400 milisegundos antes de lanzar la petición real a la API, 
mejorando el rendimiento de la aplicación
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/front c_4.png}
\end{figure}

\newpage
\section{Resultados}
La aplicacion nos permite crear libros siguiendo la estructura de la base de datos, algunas opciones pueden ser descartadas ya que no todas son obligatorias.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados1.png}
\end{figure}
Obviamente no se debe poner los comentarios que estan en el cuadro de texto, ya que marcaria un error. Se debe poner de la siguiente manera:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados5.png}
\end{figure}

\newpage
Al ponerlo de manera correcta, nos imprimira un mensaje con los datos del libro creado.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Imagenes/Resultados2.png}
\end{figure}
Nos dara la informacion de la editorial, publico objetivo, serie, autores y categorias.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagenes/Resultados3.png}
\end{figure}
\newpage
En caso de que pongamos un nombre mal en cualquier campo, nos marcara un error ya que no encontro dicho nombre.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados4.png}
\end{figure}

Podemos ver todos los libros creados en la base de datos con el endpoint de "Obtener todos los libros".
Y podemos limitar la cantidad de libros que queremos ver por medio de los Query Parameters "skip" y "limit".

Los Query Parameters estan para todos los endpoints de obtener de todos las entidades.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados6.png}
\end{figure}

\newpage
En este caso seleccionamos ver solo 1 libro:

Nos dara la misma informacion que imprime crear libro.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagenes/Resultados7.png}
\end{figure}

Es similar para los demás metodos get.

Se tiene la obtencion de libros por autor:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{Imagenes/Resultados8.png}
\end{figure}

Se tiene la obtencion de libros por categoria:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados9.png}
\end{figure}

\newpage
Se tiene la obtencion de libros por serie:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados10.png}
\end{figure}

\newpage
Se tiene la obtencion de libros por publico objetivo:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados11.png}
\end{figure}

\newpage
Para las demás entidades es similar y más sencillo, ya que solo tienen los endpoints basicos de crear, obtener por id, obtener todos, actualizar y eliminar.
Algunas opciones no eran necesarias pero se implemenaron para tener un mejor manejo y control.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Resultados12.png}
\end{figure}

\subsection*{Vista del FRONTEND:}
En esta imagen podemos observar al completo la intefaz con la que se interactuara, teniendo del lado izquierdo la seccion para agregar libros nuevos, y del 
lado derecho los libros ingresados y la barra de busqueda para consultas especificas

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 1.png}
\end{figure}

En la siguiente, podemos observar como es que ingresariamos datos nuevos para agregar libros del lado izquierdo
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 2.png}
\end{figure}

Una vez que presionemos 'crear libro', la pagina nos avisara que la operacion fue exitosa
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 3.png}
\end{figure}

En la siguiente imagen, podemos ver como funciona la busqueda por titulo, donde podremos buscar por titulos especificos
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 4.png}
\end{figure}

Despues tenemos la busqueda por autor, donde habra que escribir el nombre completo ingresado para encontrar al autor deseado
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 5.png}
\end{figure}

Posteriormente tenemos la busqueda por categorias, donde podremos buscar al menos una de las categorias ingresadas y todos los libros
que tengan dicha categoria apareceran
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 6.png}
\end{figure}

Tambien tenemos la busqueda por publico, donde podremos buscar entre aspectos como general o publicos mayores de edad
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 7.png}
\end{figure}

Por ultimo, esta la categoria de ISBN, que es un identificador unico para cada libro
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Front 8.png}
\end{figure}
