...
Se pretende crear una API centrada en la gestion del catalogo de una libreria, cuyo principal
objetivo sera proporcionar un sistema que permita la administracion de informacion de los libros y sus autores,
manejando las relaciones que hay entre ellos.
Nuestro sistema debe ser capaz de lo siguiente:
\begin{itemize}
    \item Permitir la creacion y consulta de autores en la base de datos.
    \item Permitir la creacion de nuevos libros y poder asociarlos a uno o varios autores.
    \item Implementar de forma acertada las relaciones muchos a mucho entre libros y autores.
    \item Administrar de forma eficiente la propia informacion de un libro, como lo es su editorial, 
          categorias, publico objetivo, etc.
    \item Ofrecer endpoints para poder consultar libros basados en la filtracion de diferentes criterios.
\end{itemize}


\subsection*{Propuesta de solución}
La solución propuesta es el desarrollo de una API con el uso de FastAPI, la cual usara SQLModel para gestionar la persistencia de datos.
Los componentes clave para lograr nuestra solucion son:
\begin{itemize}
    \item \textbf{SQLModel:} Para definir los modelos de datos principales como lo son las tablas SQL, ademas de una tabla de enlaces que gestionara
    la relacion muchos a muchos.
    \item \textbf{FastAPI:} Para construir la API y definir los endpoints en el modulo rutas.py, ademas de manejar las solicitudes HTTP.
    \item \textbf{Arquitectura modular:} Se refiere a la estructura que tendra la practica, dividiendola en los modulos: modelo.py, esquemas.py, servicios.py,
                 rutas.py y database.py
\end{itemize}
