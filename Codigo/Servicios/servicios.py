...
def create_libro(session: Session, libro_create: esquemas.LibroCreacion) -> modelo.Libro:
    ...
    # 2. LOGICA INTELIGENTE PARA EDITORIAL
    if libro_create.editorial_nombre:
        ...
        if not editorial:
            print(f"Editorial '{libro_create.editorial_nombre}' nueva. Creando con dirección genérica...")

            # Paso A: Crear una dirección "dummy" o genérica
            # (Porque tu BD obliga a tener dirección)
            direccion_dummy = modelo.Direccion(
                calle="Dirección Pendiente",
                ciudad_pais="Desconocido",
                codigo_postal="00000"
            )
            session.add(direccion_dummy)
            session.commit()  # Guardamos para tener ID de dirección
            session.refresh(direccion_dummy)

            # Paso B: Crear la Editorial ligada a esa dirección
            editorial = modelo.Editorial(
                nombre=libro_create.editorial_nombre,
                direccion_id=direccion_dummy.id
            )
            session.add(editorial)
            session.commit()
            session.refresh(editorial)

        # Asignamos la editorial (sea vieja o nueva) al libro
        db_libro.editorial = editorial

    # 3. Asignar Público Objetivo (Auto-creación)
    if libro_create.publico_objetivo_tipo:
        publico = get_publico_objetivo_por_tipo(session, libro_create.publico_objetivo_tipo)
        if not publico:
            publico = modelo.PublicoObjetivo(tipo=libro_create.publico_objetivo_tipo)
            session.add(publico)
            session.commit()
        db_libro.publico_objetivo = publico

    # 4. Asignar Serie (Auto-creación)
    if libro_create.serie_nombre:
        serie = get_serie_por_nombre(session, libro_create.serie_nombre)
        if not serie:
            serie = modelo.Serie(nombre=libro_create.serie_nombre)
            session.add(serie)
            session.commit()
        db_libro.serie = serie

    # 5. Asignar Autores (Auto-creación)
    for autor_nombre in libro_create.autores_nombres:
        autor = get_autor_por_nombre(session, autor_nombre)
        if not autor:
            autor = modelo.Autor(nombre=autor_nombre)
            session.add(autor)
            session.commit()
            session.refresh(autor)
        db_libro.autores.append(autor)

    # 6. Asignar Categorías (Auto-creación)
    for categoria_nombre in libro_create.categorias_nombres:
        categoria = get_categoria_por_nombre(session, categoria_nombre)
        if not categoria:
            categoria = modelo.Categoria(nombre=categoria_nombre)
            session.add(categoria)
            session.commit()
            session.refresh(categoria)
        db_libro.categorias.append(categoria)

    # 7. Guardar Libro Final
...
# --- PEGAR ESTO AL FINAL DE Servicios/servicios.py ---

# 1. Actualizar Libro (PATCH)
# --- EN Servicios/servicios.py ---

def update_libro(session: Session, libro_id: int, libro_update: esquemas.LibroActualizar) -> modelo.Libro | None:
    db_libro = session.get(modelo.Libro, libro_id)
    if not db_libro:
        return None

    # 1. Extraemos datos simples (excluyendo relaciones complejas)
    libro_data = libro_update.model_dump(
        exclude_unset=True,
        exclude={"autores_nombres", "categorias_nombres", "editorial_nombre", "publico_objetivo_tipo", "serie_nombre"}
    )
    # Actualizamos campos simples (precio, titulo, isbn)
    for key, value in libro_data.items():
        setattr(db_libro, key, value)

    # 2. Actualizar EDITORIAL (Lógica inteligente)
    if libro_update.editorial_nombre is not None:
        # Buscamos si existe la editorial por nombre
        editorial = get_editorial_por_nombre(session, libro_update.editorial_nombre)
        if not editorial:
            # Si no existe, creamos una con dirección genérica (igual que en create_libro)
            direccion_dummy = modelo.Direccion(calle="Pendiente", ciudad_pais="N/A", codigo_postal="00000")
            session.add(direccion_dummy)
            session.commit()
            editorial = modelo.Editorial(nombre=libro_update.editorial_nombre, direccion_id=direccion_dummy.id)
            session.add(editorial)
            session.commit()
        db_libro.editorial = editorial

    # 3. Actualizar PUBLICO OBJETIVO
    if libro_update.publico_objetivo_tipo is not None:
        publico = get_publico_objetivo_por_tipo(session, libro_update.publico_objetivo_tipo)
        if not publico:
            publico = modelo.PublicoObjetivo(tipo=libro_update.publico_objetivo_tipo)
            session.add(publico)
            session.commit()
        db_libro.publico_objetivo = publico

    # 4. Actualizar AUTORES
    if libro_update.autores_nombres is not None:
        db_libro.autores.clear()
        for nombre in libro_update.autores_nombres:
            autor = get_autor_por_nombre(session, nombre)
            if not autor:
                autor = modelo.Autor(nombre=nombre)
                session.add(autor)
            db_libro.autores.append(autor)

    # 5. Actualizar CATEGORÍAS
    if libro_update.categorias_nombres is not None:
        db_libro.categorias.clear()
        for nombre in libro_update.categorias_nombres:
            cat = get_categoria_por_nombre(session, nombre)
            if not cat:
                cat = modelo.Categoria(nombre=nombre)
                session.add(cat)
            db_libro.categorias.append(cat)
...
# 2. Eliminar Libro (DELETE)
def delete_libro(session: Session, libro_id: int) -> bool:
    libro = session.get(modelo.Libro, libro_id)
    if not libro:
        return False
...
# 3. Buscador por Título (Para la barra de búsqueda)
def buscar_libros_por_titulo(session: Session, texto_busqueda: str) -> List[modelo.Libro]:
    statement = select(modelo.Libro).where(col(modelo.Libro.titulo).contains(texto_busqueda))
    return session.exec(statement).all()

# 4. Contador (Para el dashboard)
def contar_libros(session: Session) -> int:
    statement = select(func.count(modelo.Libro.id))
